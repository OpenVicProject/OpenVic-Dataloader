		...
		struct MonthValue {
			...
			static constexpr auto value = dsl::callback<ast::IdentifierValue*>(
				[](ast::ParseState& state, ast::IdentifierValue* value) {
					bool is_number = true;
					for (auto* current = value->value(); *current; current++) {
			...
			dsl::callback<ast::EventStatement*>(
				[](ast::ParseState& state, NodeLocation loc, ast::IdentifierValue* name, ast::ListValue* list) {
					...
					if (name->value() != country_decl || name->value() != province_decl) {
						state.logger().warning("event declarator \"{}\" is not {} or {}", name->value(), country_decl, province_decl) //
							.primary(loc, "here")
							.finish();
					}

					return state.ast().create<ast::EventStatement>(loc, name->value() == province_decl, list);
				});
	};
