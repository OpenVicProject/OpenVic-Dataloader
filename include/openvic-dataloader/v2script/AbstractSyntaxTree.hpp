...
namespace ovdl::v2script::ast {
	...
	protected:
		explicit FlatValue(dryad::node_ctor ctor, NodeKind kind, const char* value)
			: node_base(ctor, kind),
			  _value(value) {}
	...
	struct IdentifierValue : dryad::basic_node<NodeKind::IdentifierValue, FlatValue> {
		explicit IdentifierValue(dryad::node_ctor ctor, const char* value) : node_base(ctor, value) {}
	};

	struct StringValue : dryad::basic_node<NodeKind::StringValue, FlatValue> {
		explicit StringValue(dryad::node_ctor ctor, const char* value) : node_base(ctor, value) {}
	};
	...
	struct ListValue : dryad::basic_node<NodeKind::ListValue, dryad::container_node<Value>> {
		...
		DRYAD_CHILD_NODE_RANGE_GETTER(Statement, statements, nullptr, this);
	};
	...
	struct FileTree : dryad::basic_node<NodeKind::FileTree, dryad::container_node<Node>> {
		explicit FileTree(dryad::node_ctor ctor, StatementList statements) : node_base(ctor) {
			insert_child_list_after(nullptr, statements);
		}
		...
		explicit FileTree(dryad::node_ctor ctor) : node_base(ctor) {
		}
